...
- Files changed summary: consolidated the local model setup to `docker-compose.yml`, removed duplicate `modelo/` compose/docs, simplified README quickstart commands for WSL/Linux/macOS and PowerShell, added a classroom demo prompt, and strengthened `qwen35-local` guidance for local file reads and safe multi-line file writes.
- Tests run: `docker compose config`; `docker compose up -d --remove-orphans`; container healthcheck; `qwen-codex --health`; `cd codex-rs && just fmt`; `cargo test -p codex-models-manager qwen_local_model_instructions_include_tool_guidance -- --nocapture`; `cargo build -p codex-cli`; `./scripts/install-qwen-codex.sh http://127.0.0.1:8002/v1`; local file smoke in `/tmp/qwen-codex-classroom-check`; static-site classroom smoke in `/tmp/qwen-codex-video-site`; external `python3 -m http.server` plus `curl` validation for the generated site.
